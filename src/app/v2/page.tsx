...
import {
    ...
    Trees, BookOpen, TrendingUp, Activity, 
            ...
            <section className="px-6 py-10 space-y-6">
                ...
                <div className={viewMode === "grid" ? "grid grid-cols-2 gap-4" : "space-y-4"}>
                    {isLoading ? (
                        ...
                        <FeedItem key={post.id} post={post} mode={viewMode} onClick={() => openBottomSheet("postDetail", { title: post.title })} />
                    ))}
                </div>
            </section>
...
function FeedItem({ post, mode, onClick }: { post: Post, mode: "grid" | "list", onClick: () => void }) {
    ...
    return (
        <motion.div
            whileHover={{ y: -4 }}
            className={`bg-card-bg rounded-[24px] p-5 shadow-sm border border-border cursor-pointer transition-colors duration-500 ${mode === "grid" ? "flex flex-col" : "flex flex-col"}`}
            onClick={onClick}
        >
